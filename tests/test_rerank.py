...
from websearch.rerank import MMRReranker, SupportsEmbedder
...
async def test_mmrreranker_uses_mmr_when_available(monkeypatch):
    ...
    monkeypatch.setattr("websearch.rerank.diversify_topk_mmr", fake_diversify_mmr)
...
async def test_mmrreranker_fallback_when_mmr_fails(monkeypatch):
    ...
    monkeypatch.setattr("websearch.rerank.diversify_topk_mmr", boom)

    def fake_diversify(results, k):  # type: ignore[override]
        return results[:k]

    monkeypatch.setattr("websearch.rerank.diversify_topk", fake_diversify)
